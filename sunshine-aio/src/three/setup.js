 ...
 *
 * Story 2-3 additions:
 *   - The scene holds an optional planets instance built by
 *     `createPlanetsForCategories` from `./planetFactory.js`. The factory's
 *     root group is added to the scene and its per-frame updater is wired
 *     through the same `addUpdater` registry used by the sun.
 *   - `setPlanetInstalled(planetId, installed)` flips a planet's installed
 *     flag (delegates to the factory's `setInstalled`). The state lives
...
export const createScene = (opts) => {
  ...
  // Story 2-3: optional planet factory. The factory's root Object3D is
  // added to the scene and its update function is wired through the
  // same `addUpdater` registry used by the sun. Like the sun, we keep
  // a controller-local Map of unregister handles keyed by instance so
  ...
  /**
   * Story 2-3: attach a planet factory to the scene controller. The
   * factory's root Object3D is added to the scene, and its per-frame
  ...
  const setPlanets = (factory) => {
    ...
    if (previous) {
      ...
      // cannot call into a disposed factory.
      const prevUnregister = planetUnregisterByInstance.get(previous);
      if (typeof prevUnregister === 'function') {
        prevUnregister();
      }
      planetUnregisterByInstance.delete(previous);
      // Remove the previous root from the scene so we don't leak
      // Object3D references in the scene graph after a swap.
      if (previous.group && scene.remove && typeof scene.remove === 'function') {
    ...
    planetFactoryInstance = factory || null;
    if (planetFactoryInstance) {
      if (planetFactoryInstance.group && typeof scene.add === 'function') {
        scene.add(planetFactoryInstance.group);
      }
      const unregister = addUpdater((delta, elapsed) => {
        if (typeof planetFactoryInstance.update === 'function') {
          planetFactoryInstance.update(delta, elapsed);
        }
      });
      planetUnregisterByInstance.set(planetFactoryInstance, unregister);
    }
    return previous;
  };

  const getPlanets = () => planetFactoryInstance;

  /**
   * Story 2-3: flip a planet's installed flag. No-op when no factory
   * is attached. Unknown planet ids are logged and dropped (factory's
   * `setInstalled` returns false in that case so a stale subscription
   * cannot crash the loop).
   */
  const setPlanetInstalled = (planetId, installed) => {
    if (!planetFactoryInstance || typeof planetFactoryInstance.setInstalled !== 'function') {
      return false;
    }
  ...
  const dispose = () => {
    ...
    // Story 2-3: tear down the planets factory in the same order as
    // the sun so its updater is unregistered before the registry is
    // cleared and the root group is detached from the scene graph.
    if (planetFactoryInstance) {
      const unregister = planetUnregisterByInstance.get(planetFactoryInstance);
      if (typeof unregister === 'function') {
        unregister();
      }
      planetUnregisterByInstance.delete(planetFactoryInstance);
      if (planetFactoryInstance.group && typeof scene.remove === 'function') {
        scene.remove(planetFactoryInstance.group);
      }
      if (typeof planetFactoryInstance.dispose === 'function') {
