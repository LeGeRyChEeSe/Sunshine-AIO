...
const sceneStub = {
  ...
  // Story 2-3: track parent on children so the planet factory
  // integration tests can assert on the scene graph after a swap.
  add: vi.fn((child) => {
    if (child && typeof child === 'object') {
      sceneStub.children.push(child);
      child.parent = sceneStub;
    }
  }),
  remove: vi.fn((child) => {
    const idx = sceneStub.children.indexOf(child);
    if (idx !== -1) {
      sceneStub.children.splice(idx, 1);
      if (child && typeof child === 'object') {
        child.parent = null;
      }
    }
...
vi.mock('three', () => {
  ...
  class Scene {
    constructor() {
      ...
      // Story 2-3: share add/remove/children/parent with sceneStub so
      // the planet factory integration tests can assert on the scene
      // graph and on the parent of attached groups. Each new Scene
      // instance gets a fresh children list (otherwise the mock
      // accumulates children across all createScene() calls in the
      // suite, leaking state between tests).
      this.children = [];
      this.parent = null;
      this.add = vi.fn((child) => {
        if (child && typeof child === 'object') {
  ...
  class Clock {
    constructor() {
      ...
      this.elapsedTime = clockStub.elapsedTime;
    }
  }
  return {
    WebGLRenderer,
...
const { createPlanetsForCategories } = await import('./planetFactory.js');
...
/**
 * Minimal THREE stub for `createSun` / `createPlanetsForCategories`.
 * Lives in a shared fixture (`__fixtures__/threeStub.js`) so
 * sun.test.js, planetFactory.test.js and the integration blocks
 * below cannot drift — a future assertion that touches color state
 * directly would have silently received `undefined` if the stubs
 * had diverged.
 */
...
describe('three/setup.js (Story 2-1)', () => {
  beforeEach(() => {
    ...
    sceneStub.add.mockClear();
    sceneStub.remove.mockClear();
    sceneStub.children.length = 0;
    // Story 2-3: Scenes are constructed lazily, so each new instance
    // brings its own add/remove vi.fn() that must be cleared by
    // individual tests when they want to assert on the scene graph.
  ...
  /**
   * Story 2-3 integration: the scene controller must accept a planet
   * factory, expose getPlanets / setPlanetInstalled, and tear the
   * factory down correctly on dispose and on factory swap. These
   * tests close the test-coverage gap called out in the 2-3 review.
   */
  describe('planet factory integration (Story 2-3)', () => {
    let THREE;
    ...
    const defaultCategories = [
      { id: 'games', name: 'Games', color: 0x4fc3f7, installed: false },
      { id: 'streaming', name: 'Streaming', color: 0x81c784, installed: false },
    ];
    const buildFactory = (categories = defaultCategories) =>
      createPlanetsForCategories({ THREE, categories });

    it('setPlanets attaches the factory group to the scene and registers an updater', () => {
      const controller = createScene(baseOpts());
      const factory = buildFactory();
      const group = factory.group;

      expect(controller.getPlanets()).toBeNull();

      const previous = controller.setPlanets(factory);
      expect(previous).toBeNull();
      expect(controller.getPlanets()).toBe(factory);
      // The factory's root group must be added to the scene.
      expect(group.parent).toBe(controller.scene);
      // And the per-frame driver must be registered with the loop.
      // Drive a frame and confirm the factory's update is called.
      ...
      // We need a controller whose rafFactory we can drive. Build a
      // fresh one with a captured raf, re-attach the factory.
      controller.dispose();
      const fresh = createScene(baseOpts({ rafFactory: raf }));
      const f2 = buildFactory();
      const updateSpy2 = vi.spyOn(f2, 'update');
      fresh.setPlanets(f2);
      fresh.start();
      pendingCallbacks.shift()();
      expect(updateSpy2).toHaveBeenCalledTimes(1);
      expect(updateSpy2.mock.calls[0][0]).toBe(0.016);
      expect(updateSpy2.mock.calls[0][1]).toBeGreaterThanOrEqual(0);
      // Reference spies on the discarded factory to silence linter.
      void updateSpy;
      void group;
      fresh.dispose();
    });

    it('a second setPlanets call removes the prior root from the scene and unregisters the prior updater', () => {
      const controller = createScene(baseOpts());
      const first = buildFactory();
      const second = buildFactory();
      const firstGroup = first.group;
      const secondGroup = second.group;

      controller.setPlanets(first);
      expect(firstGroup.parent).toBe(controller.scene);
      const firstDispose = vi.spyOn(first, 'dispose');

      const previous = controller.setPlanets(second);
      expect(previous).toBe(first);
      // The prior factory's group must be detached from the scene and
      // its dispose() invoked exactly once.
      expect(firstGroup.parent).toBeNull();
      expect(firstDispose).toHaveBeenCalledTimes(1);
      // The new factory's group must now be attached.
      expect(secondGroup.parent).toBe(controller.scene);
      expect(controller.getPlanets()).toBe(second);

      // Drive a frame to confirm the prior updater is no longer in
      // the loop (only the new factory's update should be called).
      ...
      controller.dispose();
      const fresh = createScene(baseOpts({ rafFactory: raf }));
      const f1 = buildFactory();
      const f2 = buildFactory();
      const f1Update = vi.spyOn(f1, 'update');
      const f2Update = vi.spyOn(f2, 'update');
      fresh.setPlanets(f1);
      fresh.setPlanets(f2);
      fresh.start();
      pendingCallbacks.shift()();
      expect(f1Update).not.toHaveBeenCalled();
      expect(f2Update).toHaveBeenCalledTimes(1);
      void firstUpdate;
      void secondUpdate;
      fresh.dispose();
    });
    ...
    it('setPlanetInstalled routes to the factory and returns false for unknown ids', () => {
      const controller = createScene(baseOpts());
      const factory = buildFactory();
      controller.setPlanets(factory);

      // Known id: returns true (the factory routes the flip).
      const setSpy = vi.spyOn(factory, 'setInstalled');
      expect(controller.setPlanetInstalled('games', true)).toBe(true);
      expect(setSpy).toHaveBeenCalledWith('games', true);

      // Unknown id: returns false (the factory rejects it).
      expect(controller.setPlanetInstalled('nope', true)).toBe(false);
    });

    it('setPlanetInstalled is a no-op when no factory is attached', () => {
      const controller = createScene(baseOpts());
      // No factory attached: should return false and not throw.
      expect(controller.setPlanetInstalled('games', true)).toBe(false);
    });

    it('dispose tears down the planet factory in the correct order', () => {
      const controller = createScene(baseOpts());
      const factory = buildFactory();
      const group = factory.group;
      ...
      controller.dispose();
      // The factory's dispose() is invoked, its group is detached,
      // and getPlanets() reports a clean state.
      expect(disposeSpy).toHaveBeenCalledTimes(1);
