...
def menu(page: int = 0):
    ...
    while True:
        ...
        if page == 0:
            ...
            print(
                "3. Configure Sunshine (only if you need to reset the preparation commands)")
...
def downloadSunshine(install: bool = True, selective: bool = False):
    downloadFile("https://github.com/LizardByte/Sunshine/releases/latest",
                 "sunshine-windows-installer", from_github=True, selective=selective)
...
def downloadVDD(install: bool = True, selective: bool = False):
    downloadFile("https://github.com/itsmikethetech/Virtual-Display-Driver/releases/latest",
                 "VDD.HDR", from_github=True, selective=selective)
...
def configureSunshine(first_install: bool = False):
    sunshine_path = findFile('C:\\Program Files\\Sunshine')
